...
extern int ntruplus576_ref_keypair(uint8_t *pk, uint8_t *sk);
extern int ntruplus576_ref_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk);
extern int ntruplus576_ref_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk);

#if defined(OQS_ENABLE_KEM_ntruplus_576_avx2)
extern int ntruplus576_avx2_keypair(uint8_t *pk, uint8_t *sk);
extern int ntruplus576_avx2_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk);
extern int ntruplus576_avx2_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk);
#endif
...
OQS_API OQS_STATUS OQS_KEM_ntruplus_576_keypair(uint8_t *public_key, uint8_t *secret_key) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_BMI2) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT)) {
		...
		return (OQS_STATUS) ntruplus576_avx2_keypair(public_key, secret_key);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) ntruplus576_ref_keypair(public_key, secret_key);
	}
#endif /* OQS_DIST_BUILD */
#else
	return (OQS_STATUS) ntruplus576_ref_keypair(public_key, secret_key);
...
OQS_API OQS_STATUS OQS_KEM_ntruplus_576_encaps(uint8_t *ciphertext, uint8_t *shared_secret, const uint8_t *public_key) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_BMI2) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT)) {
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS) ntruplus576_avx2_enc(ciphertext, shared_secret, public_key);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) ntruplus576_ref_enc(ciphertext, shared_secret, public_key);
	}
#endif /* OQS_DIST_BUILD */
#else
	return (OQS_STATUS) ntruplus576_ref_enc(ciphertext, shared_secret, public_key);
...
OQS_API OQS_STATUS OQS_KEM_ntruplus_576_decaps(uint8_t *shared_secret, const uint8_t *ciphertext, const uint8_t *secret_key) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_BMI2) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT)) {
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS) ntruplus576_avx2_dec(shared_secret, ciphertext, secret_key);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) ntruplus576_ref_dec(shared_secret, ciphertext, secret_key);
	}
#endif /* OQS_DIST_BUILD */
#else
	return (OQS_STATUS) ntruplus576_ref_dec(shared_secret, ciphertext, secret_key);
